...
impl Plugin for Simulation {
    fn build(&self, app: &mut App) {
        ...
        // Create nodes
        for k in 0..self.params.node_num_y {
            let mut vec: Vec<Entity> = Vec::new();

            for i in 0..self.params.node_num_x {
                let index = Index {
                    x: i as usize,
                    y: k as usize,
                };
...
fn setup_edges_system(commands: Commands, params: ResMut<Params>, grid: Res<Grid>) {
    setup_edges(commands, grid, params.node_num_x, params.node_num_y);
}

/// Creates edges between the nodes in Grid
fn setup_edges(mut commands: Commands, grid: Res<Grid>, node_num_x: usize, node_num_y: usize) {
    for k in 0..node_num_y {
        for i in 0..node_num_x {
            // Add top edge
            if k > 0 {
